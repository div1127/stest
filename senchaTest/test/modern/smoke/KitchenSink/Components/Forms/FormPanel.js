...
describe('FormPanel', function(){
    ...
    describe("Color Field", function() {
        ...
        it("should match field value to selector value", function() {
            ST.picker('colorfield[xtype="colorfield"]').and(function(field) {
                ST.picker('colorfield[xtype="colorfield"]').and(function(selector) {
                    expect(selector.getValue()).toBe(field.getValue());
                });
            });
        });
        ...
        it("should match swatch color to selected value", function() {
            ...
            if (ST.browser.is.IE || ST.browser.is.Edge) {
                ST.element('>> .x-colorpicker-field-swatch-inner').and(function(el) {
                    expect(el.dom.style.backgroundColor).toBe('rgba(0, 255, 0, 1)');
                });
            }
            else {
                ST.element('>> .x-colorpicker-field-swatch-inner').and(function(el) {
                    expect(el.dom.style.backgroundColor).toBe('rgb(0, 255, 0)');
                });
            }
        });
    });
