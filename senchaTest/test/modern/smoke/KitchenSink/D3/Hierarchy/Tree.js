 ...
 */
 ...
 describe("Tree", function(){
    ...
    xdescribe("Take screenshot", function(){
    ...
    // TODO: Fails on Win 10 / IE11
    xdescribe("Selecting nodes", function(){
        var counter = 20;
        for(var i = 0; i < 10; i++){
    ...
    // TODO: Fails on Win 10 / IE11
    xdescribe("Checking tooltips", function(){
        var counter = 25;
        for(var i = 0; i < 10; i++){
    ...
    xdescribe("Expanding nodes", function(){
        var counter = 20;
        for(var i = 0; i < 10; i++){
            it("Should expand node " + (i + counter) + " on doubleclick", function(){
                Tree.scene()
                    .and(function(scene){
                        Tree.nthNode(counter)
                            .and(function(node){
                                Tree.expandCollapseNode("d3-tree", counter, Tree.getScaledCoords(scene, node));
                            })
                            .wait(2 * initialAnimationDelay);
                        Tree.nthNode(counter)
                            .and(function(node){
    ...
    xdescribe("Interaction with second row", function(){
        //Open second row
        beforeAll(function(){
            Tree.scene()
                .and(function(scene){
                    Tree.nthNode(28)
                        .and(function(node){
                            Tree.expandCollapseNode("d3-tree", 28, Tree.getScaledCoords(scene, node));
                    });
                })
                .wait(2 * initialAnimationDelay);
        });
        ...
        //number of nodes, root + 51 US states
        //I don't expect this number to change anytime soon, hence hardcoded
        var counter = 53;
        for(var i = 0; i < 5; i++){
            it("Should click on node " + (i + counter), function(){
                Tree.scene()
                .and(function(scene){
                    Tree.nthNode(counter)
                        .and(function(node){
                            Lib.D3.clickOnCoordinates("d3-tree", counter++, undefined ,Tree.getScaledCoords(scene, node));
                    });
                });
            });
        }
        ...
        for(var i = 0; i < 5; i++){
            it("Should check tooltip on node " + (i + counter), function(){
                Tree.scene()
                .and(function(scene){
                    Tree.nthNode(counter)
                        .and(function(node){
    ...
    describe("Checking drag support", function(){
        it("Should drag", function(){
            pending('Disabled: Failed with error "Unable to get property \'length\' of undefined or null reference"');
