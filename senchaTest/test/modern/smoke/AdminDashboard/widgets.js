/**
 * @file widgets.js
 * @author Jakub Janoska
 * @edited by Vojtech Cerveny
 * @date 16.8.2016
 *
...
describe('Widgets Page', function () {
    ...
    function testButtons (element,button,order) {
        it('Click on ' + button + ' button on ' + element, function () {
            ...
            ST.play([
                // press button
                { type: "mousedown", target: button1, detail: 1 }
            ]);
            ST.button(button1)
                .and(function (button) {
                    expect(button.el.dom.className).toContain('x-pressing');
                });
            ST.play([
                // unpress button
                { type: "mouseup", target: button1, detail: 1 }
            ]);
        });
    }
    ...
    beforeEach(function () {
        ...
        Lib.beforeAll("#widgets", "widgets", undefined, "admin");
    });
    afterAll(function(){
        //Lib.afterAll("widgets");//It is not need destroyed
