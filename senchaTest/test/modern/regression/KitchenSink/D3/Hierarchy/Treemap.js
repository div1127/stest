 ...
 */
 ...
 describe("Treemap example", function(){
    ...
    // TODO: Fails on multiple browsers with a timeout.  if you run just a few tests you can get them to pass
    xdescribe("Clicking on nodes", function(){
        var counter =22;
        for(var i = 0; i < 10; i++){
    ...
    // TODO: Timeout on multiple browsers, this is fairly consistent, but you can get them to pass if you run them enough
    xdescribe("Hovering over nodes", function(){
        var counter = 32;
        for(var i = 0; i < 10; i++){
    ...
    // TODO: Timeout on multiple browsers
    ...
    xdescribe("Double clicking on nodes", function(){
        it("Should expand/collapse node Services", function(){
            Treemap.panelComponent()
                .and(function(panel){
                    Treemap.nodeElement(4)
                        .and(function(node){
                            var coords = Treemap.tupleFromText(node.dom.attributes.transform.value);
                            Treemap.treemapItself()
                                .and(function(){
                                    Lib.Chart.doubleClick("panel[id=" + panel.getId() + "]", coords.x, coords.y);
                                });
                                ST.wait(function(){
                                    return (node.dom.className.baseVal.indexOf("x-d3-expanded") == -1);
                                })
                                .and(function(){
                                    expect(node.dom.className.baseVal.indexOf("x-d3-expanded")).toBe(-1);
                                });
                        });
                    ...
                    Treemap.nodeElement(4)
                        .and(function(node){
                            ...
                            Treemap.treemapItself()
                                ...
                                .and(function(){
                                    expect(node.dom.className.baseVal.indexOf("x-d3-expanded")).not.toBe(-1);
                                });
                            });
                    });
