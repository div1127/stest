 ...
 * By Jiri Znoj - last update: 19. 07. 2017
 * tested on:
 *          desktop:
 *              Chrome 59
 *              Firefox 54
...
describe("Pack", function(){
    ...
    xit("Verify loading and verify count of 16 root items", function(){
        ST.wait(function(){
    ...
    // TODO: Fails on Win 10 / IE 11
    xdescribe("Clicking on nodes and check x-d3-selected (without Root, because could click on something else)", function(){
    ...
    // TODO: Consistent timeouts across multiple browsers.
    xdescribe("Mouseover and check if tooltip text fits", function(){
    ...
    xdescribe("Doubleclick on nodes", function(){
        [0, 11, 14].forEach(function(i){
            //don't have test for collapse, because it is flacky - different resolutions
            //change positions of children, so there is no way how to proceed every time succesfully
            it("Should expand node " + nodeNames[i], function(){
                ST.component(componentName)
                    .and(function(d3p) {
                        wasExpanded = d3p.getStore().root.childNodes[i].data.expanded;
                        if (wasExpanded) {
                            Pack.collapseIfNotExpanded(i);
                        }
                    })
                    .wait(Lib.D3.waitDuration(componentName));
                var paddingCoords;
                ST.element(componentName).down('>>g')
                    .and(function(node){
                        paddingCoords = Lib.D3.tupleFromText(node.dom.attributes.transform.value);
                        Lib.D3.expandCollapseNode(componentName, i+2, paddingCoords.x, paddingCoords.y);
                    })
                    .wait(Lib.D3.waitDuration(componentName));
            });
        });
    ...
    xit("Check if opened nodes stays opened when are hidden by counting visible circles", function(){
        //open geo (test opened)
        var paddingCoords;
        ST.element(componentName).down('>>g')
            .and(function(node){
                paddingCoords = Lib.D3.tupleFromText(node.dom.attributes.transform.value);
                Lib.D3.expandCollapseNode(componentName, 27, paddingCoords.x);
            })
            .wait(Lib.D3.waitDuration(componentName))
            .and(function(){
                expect(Ext.first(componentName).getStore().getData().length).toBe(106);
            });

        //collapse test (geo opened inside)
        ST.element(componentName + " => .x-d3-node:nth-child(2)")
            .and(function(node){
                var r = node.dom.firstChild.r.baseVal.value;
                Lib.D3.expandCollapseNode(componentName, 2, paddingCoords.x, paddingCoords.y+4-r);
            })
            .wait(Lib.D3.waitDuration(componentName))
            .and(function(){
                expect(Ext.first(componentName).getStore().getData().length).toBe(50);

                //open test (geo should be still opened)
                Lib.D3.expandCollapseNode(componentName, 2, paddingCoords.x);
            })
            .wait(Lib.D3.waitDuration(componentName))
            .and(function(){
                expect(Ext.first(componentName).getStore().getData().length).toBe(106);
        ...
        //collapse geo (test is still opened)
        ST.element(componentName + " => .x-d3-node:nth-child(" + (27) + ")")
            .and(function(node){
                var r = node.dom.firstChild.r.baseVal.value;
                Lib.D3.expandCollapseNode(componentName, 27, paddingCoords.x+4-r, paddingCoords.y);
            })
            .wait(Lib.D3.waitDuration(componentName))
            .and(function(){
                expect(Ext.first(componentName).getStore().getData().length).toBe(74);
            })
