 ...
 * By Jiri Znoj - last update: 24. 10. 2016
 * tested on:
 *          desktop:
 *              Chrome 52, 54, 55
...
describe("TreeList", function() {
    ...
    describe('Expand and Collapse all items when Micro is pressed, see if chosen items are correctly selected and correct path is present', function(){
        ...
        xdescribe('Click on Home button, expand, collapsed and check if done correcly - \'Archive\'', function(){
            beforeAll(function() {
                ST.play([
                    { type: "tap", target: "treelist", x: 20, y: 20 }
                ]);
            });
            Lib.expandElement(Components.treeListItem('Archive'), 21, true);
            describe('Open Archive, select \'First\' and see if selected', function(){
                beforeAll(function() {
                    expandCollapse(ST.element(Components.treeListItem('Archive')));
                });
                afterAll(function() {
                ...
                selectAndCheckElement(Components.treeListItem('First'), '\/Home\/Archive\/First', 22);
            });
        });
        xdescribe('Click on Settings, select \'Network\' and see if selected', function(){
            beforeAll(function() {
                ST.play([
                    { type: "tap", target: "treelist", x: 20, y: 7*(176/8) }
                ]);
            });
            selectAndCheckElement(Components.treeListItem('Network'), '\/Settings\/Network', 26);
        });
