...
describe('FieldValidation', function(){
    ...
    describe('Date field', function() {
        ...
        it('required', function() {
            Form.button('Submit').click();
            Lib.Forms.validateError('field[_label='+fieldLabels[2]+']', false, '', 'This field is required');    
        });
        it('tooltip is displayed', function() {
            Form.button('Submit').click();
            Lib.Forms.tooltipError('field[_label='+fieldLabels[2]+']', 'This field is required');
            Form.fieldInput(2).type('wrong');
            Lib.Forms.tooltipError('field[_label='+fieldLabels[2]+']', 
                'Value does not match the required format');
        });
        xit('valid input', function() {
            Lib.DatePanel.pickDate('field[_label='+fieldLabels[2]+']', 23, 11, 2020);
            Lib.Forms.validateField('field[_label="Date of Birth"]', 23, 11, 2020);
            Form.field(2).and(function(field){
        ...
        it('invalid format', function() {
            Lib.Forms.validateInput('field[_label='+fieldLabels[2]+']', validInputs[2]);
            Form.resetField(2);
            Lib.Forms.validateError('field[_label='+fieldLabels[2]+']', false, 'wrong', 'Value does not match the required format');
        });
    });
    ...
    //InputMask tests fail if browser window is not focused
    describe('Phone number field - ORION-1927 & ORION-2202', function() {
        afterEach(function(){
            Form.resetField(4);   
        });
        xit('input mask', function() {
            Form.fieldInput(4)
                .focus()
                .and(function(field) {
                    expect(field.dom.value).toBe('(___) ___-____');        
                });
        });
        it('invalid format', function() {
            Lib.Forms.validateError('field[_label='+fieldLabels[4]+']', false, '23', 'Value does not match the required format');
        });
        it('tooltip is displayed', function() {
            Form.fieldInput(4).type('23');
            Lib.Forms.tooltipError('field[_label='+fieldLabels[4]+']', 
                'Value does not match the required format');
        });
        it('valid input - ORION-2202', function() {
            Lib.Forms.validateError('field[_label='+fieldLabels[4]+']', true, validInputs[4]);
            Form.resetField(4);
            Lib.Forms.validateInput('field[_label='+fieldLabels[4]+']', validInputs[4], '(123) 456-7890');
        });
        it('clear field', function() {
            Form.fieldInput(4)
                .type('23');
